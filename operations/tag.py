#!/usr/bin/env python3
"""
A module containing the "tag" function in addition to several helper functions.


Pseudo-code for reference:

create file manipulator
create timestamp and extraction folder
put files to tag into videoFiles array

for all files in videoFiles:
    get tag data
    create mkvpropedit command object
    set manual overrides (via command arguments)

    for tracks in tag data:
        add track to trackLangPairs if type == subtitles

    for each track in trackLangPairs:
        create extract command object

        if lang is und and no override:
            create exported sub file and extract subs
            read subs into langdetect
            translate langdetect designation into ISO 639_2
            tag track in file
        elif lang is und and yes override:
            tag track with override

run full tag command
cleanup operational files
...
def tag(vidsrc: pathlib.Path() = pathlib.Path.cwd(),
    ...
    if vidsrc.is_dir():
        videoFiles = tagOp.parse_dir(globPattern = "*.mkv",
    ...
    for srcfile in videoFiles:
        ...
        for track, lang in trackLangPairs.items():
            ...
            if (lang == 'und' and subs_lang == None) or (force_check):
